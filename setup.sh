#!/usr/bin/env bash
# =============================================================================
# One-command setup for Twitter Automation Agent
...
AGENT_NAME="Twitter Automation Agent"
AGENT_ID="twitter-automation-agent"
...
phase_env() {
  ...
  "GOOGLE_SHEETS_CLIENT_EMAIL|Google Sheets client email|Google service account client email with access to the content queue spreadsheet.|Open https://console.cloud.google.com/ in your browser\nCreate or choose a Google Cloud project\nClick 'APIs & Services', then enable the Google Sheets API\nClick 'IAM & Admin', then 'Service Accounts'\nCreate or open a service account and create a JSON key\nCopy the client_email value and paste it into the field above."
  "GOOGLE_SHEETS_PRIVATE_KEY|Google Sheets private key|Google service account private key used with the client email for Google Sheets API access.|Open https://console.cloud.google.com/ in your browser\nOpen the Google Cloud project used for this agent\nClick 'IAM & Admin', then 'Service Accounts'\nOpen the service account and click 'Keys'\nCreate or download a JSON key\nCopy the private_key value and paste it into the field above."
  "TWITTER_API_KEY|Twitter/X API key|Twitter/X API key for the app that publishes queued posts.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nCreate or open an app with posting permissions\nOpen the app's 'Keys and tokens' page\nCopy the API Key and paste it into the field above."
  "TWITTER_API_SECRET|Twitter/X API secret|Twitter/X API secret for the app that publishes queued posts.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nCopy the API Key Secret and paste it into the field above."
  "TWITTER_ACCESS_TOKEN|Twitter/X access token|Twitter/X user access token for the account that publishes queued posts.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nGenerate user access tokens with posting permissions\nCopy the Access Token and paste it into the field above."
  "TWITTER_ACCESS_TOKEN_SECRET|Twitter/X access token secret|Twitter/X user access token secret for the account that publishes queued posts.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nGenerate user access tokens with posting permissions\nCopy the Access Token Secret and paste it into the field above."
  "TWITTER_API_KEY|Twitter/X publishing API key|Twitter/X publishing channel API key.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nCreate or open an app with posting permissions\nOpen the app's 'Keys and tokens' page\nCopy the API Key and paste it into the field above."
  "TWITTER_API_SECRET|Twitter/X publishing API secret|Twitter/X publishing channel API secret.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nCopy the API Key Secret and paste it into the field above."
  "TWITTER_ACCESS_TOKEN|Twitter/X publishing access token|Twitter/X publishing channel access token for the posting account.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nGenerate user access tokens with posting permissions\nCopy the Access Token and paste it into the field above."
  "TWITTER_ACCESS_TOKEN_SECRET|Twitter/X publishing access token secret|Twitter/X publishing channel access token secret for the posting account.|Open https://developer.x.com/ in your browser\nSign in and open the Developer Portal\nOpen the app used for this agent\nOpen the app's 'Keys and tokens' page\nGenerate user access tokens with posting permissions\nCopy the Access Token Secret and paste it into the field above."
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/twitter-automation-agent"
      exit 1
    fi
...
phase_finish() {
  ...
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^google-sheets-row-event\b"; then
      ok "cron 'google-sheets-row-event' already registered"
    else
      openclaw cron add --file cron/google-sheets-row-event.json && ok "registered cron 'google-sheets-row-event'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/google-sheets-row-event.json"
  fi
  ...
  log "  openclaw cron run --name google-sheets-row-event    # trigger manually"
